            ...
            <div id="layoutSidenav_content">
                ...
                <main>
                    <div class="container-fluid">
                        ...
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item active">Formulaire pour créer un droit</li>
                        </ol>
                        <div class="row">
                            <form class="custom_display" id="form_crud" action="formulaire_droit.php" method="post">
                                <div></div>
                                <div>
                                    <label for="autorisationName">Nom du droit :</label>
                                    <input type="text" id="autorisationName" name="autorisationName" required>
                ...
                <?php /************************ Si aucune connexion à la base de donnée, redirection vers la page d'erreur du serveur ******************************/
                                        ...
                                        if(isset($_POST['submit'])){
                                            $autorisationname = trim($_POST['autorisationName']);
                                            ...
                                            // Vérification si le nom du droit n'est pas numérique
                                            if($isValid && is_numeric($autorisationname)) {
                                                $isValid = false;
                                                $error_message = "Le nom du droit ne peut être écrit en numérique.";
                                                echo '<p class="error">'.$error_message.'</p>';
                                            }
                                        
                                                // Enregistrement dans la base de données
                                                if($isValid){
                                                    $insertSQL = "INSERT INTO droit(nom_droit) values(?)";
                                                    $stmt = $mysqli->prepare($insertSQL);
                                                    $stmt->bind_param("s",$autorisationname);
                                                    $stmt->execute();
                                                    $stmt->close();
                                            
                                                $success_message = "Le nouveau droit a été créé avec succès. Vous allez être redirigé vers la page d'accueil dans quelques secondes.";
