    ...
    <body class="sb-nav-fixed">
                ...
                <main>
                    <div class="container-fluid">
                        ...
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item active">Formulaire pour créer un type d'action</li>
                        </ol>
                        <div class="row">
                            <form class="custom_display" id="form_crud" action="formulaire_typeaction.php" method="post">
                                <div></div>
                                <div>
                                    <label for="typeaction">Nom du type d'action:</label>
                                    <input type="text" id="typeaction" name="typeaction" required>
      ...
      <?php /************************ Si aucune connexion à la base de donnée, redirection vers la page d'erreur du serveur ******************************/
                                        ...
                                        if(isset($_POST['submit'])){
                                            ...
                                            if($isValid && is_numeric($typeaction)) {
                                            $isValid = false;
                                            $error_message = "Le nom du type d'action ne doit pas être un chiffre ou un nombre.";
                                            echo '<p class="error">'.$error_message.'</p>';
                                            }
                                                                                    
                                                if($isValid){
                                            
                                                    // Vérification si le nom du type d'action existe déjà dans la BDD
                                                    $stmt = $mysqli->prepare("SELECT libelle_typeaction FROM typeaction WHERE libelle_typeaction = ?");
                                                    $stmt->bind_param("s", $typeaction);
                                                    $stmt->execute();
                                                ...
                                                if($isValid){
                                                    $insertSQL = "INSERT INTO typeaction (libelle_typeaction) values(?)";
                                                    $stmt = $mysqli->prepare($insertSQL);
                                                    $stmt->bind_param("s",$typeaction);
                                                    $stmt->execute();
                                                    $stmt->close();
                                            
                                                $success_message = "Le type d'action a été créé avec succès. Vous allez être redirigé vers la page d'accueil dans quelques secondes.";
