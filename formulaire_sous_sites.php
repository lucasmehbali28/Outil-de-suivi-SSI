        ...
        <div id="layoutSidenav">
                ...
                <main>
                    <div class="container-fluid">
                        ...
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item active">Formulaire pour créer un sous site</li>
                        </ol>
                        <div class="row">
                            <form class="custom_display" id="form_crud" action="formulaire_sous_sites.php" method="post">
                                <div></div>
                                <div>
                                    <label for="subSiteName">Nom du sous site :</label>
                                    <input type="text" id="subSiteName" name="subSiteName" required>
         ...
         <?php /************************ Si aucune connexion à la base de donnée, redirection vers la page d'erreur du serveur ******************************/
                                        ...
                                        if(isset($_POST['submit'])){
                                            $subsitename = trim($_POST['subSiteName']);
                                            ...
                                            if($isValid && is_numeric($subsitename)) {
                                            $isValid = false;
                                            $error_message = "Le nom du sous_site ne peut être un chiffre ou un nombre.";
                                            echo '<p class="error">'.$error_message.'</p>';
                                            }
                                            
                                                // Vérification si le sous site n'existe pas dans la BDD
                                                if($isValid){
                                                    $stmt = $mysqli->prepare("SELECT nom_sous_sites FROM sous_sites WHERE nom_sous_sites = ?");
                                                    $stmt->bind_param("s", $subsitename);
                                                    $stmt->execute();
                                                    $result = $stmt->get_result();
                                                    $stmt->close();
                                                    if($result->num_rows > 0){
                                                        $isValid = false;
                                                        $error_message = "Ce nom de sous site existe déjà.";
                                                        echo '<p class="error">'.$error_message.'</p>';
                                                    }
                                                }
                                                ...
                                                if($isValid){
                                                    $insertSQL = "INSERT INTO sous_sites (nom_sous_sites,id_parent_sous_sites,id_entite_sous_sites) values(?,?,?)";
                                                    $stmt = $mysqli->prepare($insertSQL);
                                                    $stmt->bind_param("sss",$subsitename,$entite,$site);
                                                    $stmt->execute();
                                                    $stmt->close();
                                            
                                                $success_message = "Le sous site a été créé avec succès. Vous allez être redirigé vers la page d'accueil dans quelques secondes.";
