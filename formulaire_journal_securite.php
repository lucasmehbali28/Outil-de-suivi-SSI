...
<html lang="FR-fr">
                ...
                <main>
                    <div class="container-fluid">
                        ...
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item active">Formulaire journal de sécurité</li>
                        </ol>
                        <div class="row">
                            <form class="custom_display" id="form_crud" action="formulaire_journal_securite.php" method="post">
                                <div>
                                        ...
                                        <select name="place" id="place" >
                                            ...
                                            // L'idée est de récupérer l'id parent du sous site pour filtrer les sous sites liés au parent [Nécessite Javascript select/option]
                                                while ($row = $result->fetch_array()): ?>
                                            <option value="<?php echo $row['id_sous_sites']?>"><?php echo $row['nom_sous_sites']?></option>   
                                                <?php endwhile; ?>
                                        </select>

                                        <label for="durationOfTheCut">Durée de la coupure :</label>
                                        ...
                                        <label for="numberAffectByTheCut">Nombre de personnes impactés:</label>
                                        ...
                                        <select name="type" id="type" required>
                                            ...
                                            <option value="incident_operateur">Incident Opérateur</option>
                                        ...
                                        <label for="etpEstimatedCost">Coût ETP estimé en €:</label>
                                        <input type="number" id="etpEstimatedCost" name="etpEstimatedCost" min='1' required>
                                  
                                        <label for="financialCost">Coût Financier en €:</label>
    ...
    <?php
    /* test sur le format de la date en décomposant les jour/mois/année 
    ...
    var_dump(checkdate($month,$day,$year)); // boolean TRUE si -> MOIS - JOUR - ANNEE *
                        ...
                        /************************ Si aucune connexion à la base de donnée, redirection vers la page d'erreur du serveur ******************************/
                                if (!$mysqli) {?>
                                    <script>
                                        setTimeout(function () {
                                ...
                                if(isset($_POST['submit'])){
                                    ...
                                    // Vérification si l'année du formulaire n'est pas inférieure à l'année N
                                    if($isValid && $year < date('Y')) {
                                    $isValid = false;
                                    $error_message = "L'année doit être supérieure ou égale à '".date('Y')."' ";
                                    echo '<p class="error">'.$error_message.'</p>';
                                    }
                                
                                    // Vérification de la date de fin d'incident
                                    if ($isValid && $enddateofincident <= $reportingdate) {
                                        $isValid = false;
                                        $error_message = "La date de fin d'incident ne coïncide pas avec la date de signalement de l'incident.";
                                        echo '<p class="error">'.$error_message.'</p>';
                                    }

                                    // Vérification des input numériques
                                    if ($isValid && !is_numeric($numberaffectbythecut) || !is_numeric($ticketglpi) || !is_numeric($etpestimatedcost) || !is_numeric($financialcost)) {
                                        ...
                                        $success_message = "Le rapport d'incident a été créé avec succès. Vous allez être redirigé vers la page d'accueil dans quelques secondes.";
