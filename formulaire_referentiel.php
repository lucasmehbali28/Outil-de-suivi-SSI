                ...
                <main>
                    <div class="container-fluid">
                        ...
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item active">Formulaire pour intégrer un nouveau référentiel</li>
                        </ol>
                        <div class="row">
                            <form class="custom_display" id="form_crud" action="formulaire_referentiel.php" method="post">
                                <div></div>
                                <div>
                                    <label for="refName">Nom du référentiel :</label>
                                    <input type="text" id="refName" name="refName" required>

                                    <label for="refLimitDate">Date de validité du référentiel :</label>
                                    <input type="date" id="refLimitDate" name="refLimitDate" required>

                                    <label for="refUrl">URL du référentiel :</label>
                                    <input type="url" id="refUrl" name="refUrl" placeholder="https://example.fr" pattern="^(?!((([A-Za-z]{3,9}:(?:\/\/)?)(?:[-;:&=\+\$,\w]+@)?[A-Za-z0-9.-]+|(?:www.|[-;:&=\+\$,\w]+@)[A-Za-z0-9.-]+)((?:\/[\+~%\/.\w-_]*)?\??(?:[-\+=&;%@.\w_]*)#?(?:[\w]*))?))" minlength="10" maxlength="80" required>
                    ...
                    <?php /************************ Si aucune connexion à la base de donnée, redirection vers la page d'erreur du serveur ******************************/
                                        ...
                                        if(isset($_POST['submit'])){
                                            ...
                                            $reflimitdate = trim($_POST['refLimitDate']);
                                            $refUrl = trim($_POST['refUrl']);
                                            
                                            $isValid = true;
                                        
                                            // Vérification si les champs sont remplis
                                            if($refname == '' || $reflimitdate == '' || $refUrl== '' ){
                                            $isValid = false;
                                            ...
                                            echo '<p class="error">'.$error_message.'</p>';
                                                ...
                                                if($isValid){
                                                    $insertSQL = "INSERT INTO referentiel(nom_referentiel,date_validite_referentiel,url_referentiel) values(?,?,?)";
                                                    $stmt = $mysqli->prepare($insertSQL);
                                                    $stmt->bind_param("sss",$refname,$reflimitdate,$refUrl);
                                                    $stmt->execute();
                                                    $stmt->close();
                                            
                                                $success_message = "Le référentiel a été créé avec succès. Vous allez être redirigé vers la page d'accueil dans quelques secondes.";
