                ...
                <main>
                    <div class="container-fluid">
                        ...
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item active">Formulaire niveaux de sécurité</li>
                        </ol>
                        <div class="row">
                            <form class="custom_display" id="form_crud" action="formulaire_niveaux_securite.php" method="post">
                                <div></div>
                                <div>
                                    <label for="type">Type d'élément:</label>
                                      ...
                                      <option value="">--Sélectionnez une action--</option>   
                                        <?php $result = $mysqli->query('SELECT id_action, titre_action FROM action');
                                            while ($row = $result->fetch_array()): ?>
                                        <option value="<?php echo $row['id_action']?>"><?php echo $row['titre_action']?></option>
                                            <?php endwhile; ?>
                                      </select>
                    ...
                    <?php /************************ Si aucune connexion à la base de donnée, redirection vers la page d'erreur du serveur ******************************/
                                        ...
                                        if(isset($_POST['submit'])){
                                            ...
                                            if($isValid && is_numeric($description) || is_numeric($title)) {
                                            $isValid = false;
                                            $error_message = "Veuillez saisir du texte.";
                                            echo '<p class="error">'.$error_message.'</p>';
                                            }
                                            
                                                // Enregistrement dans la base de données
                                                if($isValid){
                                                    $insertSQL = "INSERT INTO niveaux_securite (type_niveaux_securite,libelle_niveaux_securite,description_niveaux_securite,niveau_niveaux_securite,id_action_niveaux_securite) values(?,?,?,?,?)";
                                                    $stmt = $mysqli->prepare($insertSQL);
                                                    $stmt->bind_param("sssss",$type,$level,$title,$description,$action);
                                                    $stmt->execute();
                                                    $stmt->close();
                                            
                                                $success_message = "Le rapport de niveaux de sécurité a été créé avec succès. Vous allez être redirigé vers la page d'accueil dans quelques secondes.";
