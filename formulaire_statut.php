            ...
            <div id="layoutSidenav_content">
                ...
                <main>
                    <div class="container-fluid">
                        ...
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item active">Formulaire pour créer un statut</li>
                        </ol>
                        <div class="row">
                            <form class="custom_display" id="form_crud" action="formulaire_statut.php" method="post">
                                <div></div>
                                <div>
                                    <label for="statute">Nom du statut :</label>
                                    <input type="text" id="statute" name="statute" required>
             ...
             <?php /************************ Si aucune connexion à la base de donnée, redirection vers la page d'erreur du serveur ******************************/
                                        ...
                                        if(isset($_POST['submit'])){
                                            ...
                                            // Vérification si le nom du statut n'est pas un chiffre/nombre
                                            if($isValid && is_numeric($statute)) {
                                            $isValid = false;
                                            $error_message = "Le statut n'est pas un chiffre ou un nombre.";
                                            echo '<p class="error">'.$error_message.'</p>';
                                            }
                                                                                    
                                                if($isValid){
                                            
                                                    // Vérification si le statut existe en BDD
                                                    $stmt = $mysqli->prepare("SELECT libelle_statut FROM statut WHERE libelle_statut = ?");
                                                    $stmt->bind_param("s", $statute);
                                                    $stmt->execute();
                                                    $result = $stmt->get_result();
                                                    $stmt->close();
                                                    if($result->num_rows > 0){
                                                        $isValid = false;
                                                        $error_message = "Ce statut existe déjà.";
                                                        echo '<p class="error">'.$error_message.'</p>';
                                                    }
                                                }
                                                ...
                                                if($isValid){
                                                    $insertSQL = "INSERT INTO statut (libelle_statut) values(?)";
                                                    $stmt = $mysqli->prepare($insertSQL);
                                                    $stmt->bind_param("s",$statute);
                                                    $stmt->execute();
                                                    $stmt->close();
                                            
                                                $success_message = "Le statut a été créé avec succès. Vous allez être redirigé vers la page d'accueil dans quelques secondes.";
