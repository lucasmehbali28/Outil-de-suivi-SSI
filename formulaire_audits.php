                ...
                <main>
                    <div class="container-fluid">
                        ...
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item active">Formulaire pour les audits</li>
                        </ol>
                        <div><!--div class="row"-->
                            <form class="custom_display" id="form_crud" action="formulaire_audits.php" method="post">
                                <div></div>
                                <div>
                                  <label for="reportingDate">Date de l'audit:</label>
                                  ...
                                  <select name="type" id="type" required>
                                      ...
                                      <option value="2">Contrôle d'accès</option>
                                      <option value="3">Serveur</option>
                                  </select>

                                  <label for="numberAffect">Nombre d'agents concernés:</label>
                                  <input type="number" id="numberAffect" name="numberAffect" min="1" required>

                                  <label for="support">Supports :</label>
                                  <input type="text" id="support" name="support" required>
                                
                                  <label for="resultat">Résultat audit</label>
                                  <textarea id="resultat" name="resultat" required></textarea>

                                  <label for="livrable">Livrable audit</label>
                                  <textarea id="livrable" name="livrable" required></textarea>

                                  <label for="actionAfterAudit">Actions suite à l'audit</label>
                                  <textarea id="actionAfterAudit" name="actionAfterAudit" required></textarea>
...
<?php /************************ Si aucune connexion à la base de donnée, redirection vers la page d'erreur du serveur ******************************/
                                ...
                                if(isset($_POST['submit'])){
                                    ...
                                    $numberaffect = trim($_POST['numberAffect']);
                                    $support = trim($_POST['support']);
                                    $resultat = trim($_POST['resultat']);
                                    $livrable = trim($_POST['livrable']);
                                    $actionafteraudit = trim($_POST['actionAfterAudit']);

                                    $isValid = true;

                                    // Vérification si les champs sont remplis
                                    if($reportingdate == '' || $title == '' || $description == '' || $type == ''|| $numberaffect  == '' || $support == '' || $resultat == '' || $livrable == '' || $actionafteraudit == '' ){
                                        $isValid = false;
                                        $error_message = "Veuillez remplir tout les champs.";
                                        echo '<p class="error">'.$error_message.'</p>';
                                        }
                                        
                                        // Vérification si le nombre d'agents concernés est respecté
                                        if ($isValid && !is_numeric($numberaffect)) {
                                            $isValid = false;
                                            $error_message = "Vous devez saisir un nombre pour désigner le nombre d'agents concernés .";
                                            echo '<p class="error">'.$error_message.'</p>';
                                        }
                                                                                
                                            // Enregistrement dans la base de données
                                            if($isValid){
                                                $insertSQL = "INSERT INTO audits (date,libelle,description,type,nombre_agents_concernes,supports,resultat,livrable,actions_suite_audit) values(?,?,?,?,?,?,?,?,?)";
                                                $stmt = $mysqli->prepare($insertSQL);
                                                $stmt->bind_param("sssssssss",$reportingdate,$title,$description,$type,$numberaffect,$support,$resultat,$livrable,$actionafteraudit);
                                                $stmt->execute();
                                                $stmt->close();
