                ...
                <main>
                    <div class="container-fluid">
                        ...
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item active">Formulaire pour les actions de communications</li>
                        </ol>
                        <div class="row">
                            <form class="custom_display" id="form_crud" action="formulaire_communication.php" method="post">
                                <div></div>
                                <div>
                                  ...
                                  <select name="type" id="type" required>
                                      ...
                                      <option value="1">Mails</option>
                                      <option value="2">Formations</option>
                        ...
                        if(isset($_POST['submit'])){
                            ...
                            if($reportingdate == '' || $title == '' || $description == '' || $type == ''|| $numberaffect == '' || $support == '' ){
                            $isValid = false;
                            ...
                            if($isValid && date('Y-m-d') > $reportingdate){
                            $isValid = false;
                            $error_message = "Vous ne pouvez saisir une date antérieure à aujourd'hui.";
                            echo '<p class="error">'.$error_message.'</p>';
                            }
                            ...
                            // Vérification des champs non supposés être numérique
                            if ($isValid && is_numeric($title) || is_numeric($description)) {
                                $isValid = false;
                                $error_message = "Vous ne pouvez écrire qu'en numérique.";
                                echo '<p class="error">'.$error_message.'</p>';
                            }
                                                       
                                // Enregistrement dans la base de données
                                if($isValid){
                                    $insertSQL = "INSERT INTO communications (date,libelle,description,type_action,nombre_agents_concernes,supports) values(?,?,?,?,?,?,?)";
                                    $stmt = $mysqli->prepare($insertSQL);
                                    $stmt->bind_param("sssssss",$reportingdate,$title,$description,$type,$numberaffect,$support);
                                    $stmt->execute();
                                    $stmt->close();
                            
                                $success_message = "Le rapport de communication a été créé avec succès. Vous allez être redirigé vers la page d'accueil dans quelques secondes.";
