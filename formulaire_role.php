                ...
                <main>
                    <div class="container-fluid">
                        ...
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item active">Formulaire pour créer un nouveau rôle</li>
                        </ol>
                        <div class="row">
                            <form class="custom_display" id="form_crud" action="formulaire_role.php" method="post">
                                <div></div>
                                <div>
                                    <label for="roleName">Nom du rôle :</label>
                                    <input type="text" id="roleName" name="roleName" required>

                                    <label for="autorisation">Droits :</label>
                                        <select name="autorisation" id="autorisation" required>
                                        <?php $result = $mysqli->query('SELECT id_droit, nom_droit FROM droit');
                                            while ($row = $result->fetch_array()): ?>
                                        <option value="<?php echo $row['id_droit']?>"><?php echo $row['nom_droit']?></option>
                                            <?php endwhile; ?>
                                    </select> 
                                    ...
                                    <?php /************************ Si aucune connexion à la base de donnée, redirection vers la page d'erreur du serveur ******************************/
                                        ...
                                        if(isset($_POST['submit'])){
                                            ...
                                            if($rolename == '' || $autorisation == '' ) {
                                            $isValid = false;
                                            $error_message = "Veuillez remplir tous les champs.";
                                            echo '<p class="error">'.$error_message.'</p>';
                                            }
                                        
                                            // Vérification si le nom du rôle n'est pas un chiffre
                                            if($isValid && is_numeric($rolename)) {
                                            $isValid = false;
                                            $error_message = "Le nom du rôle n'est pas un chiffre ou un nombre.";
                                            echo '<p class="error">'.$error_message.'</p>';
                                            }
                                        
                                                // Enregistrement dans la base de données
                                                if($isValid){
                                                    $insertSQL = "INSERT INTO role (nom_role,id_droit_role) values(?,?)";
                                                    $stmt = $mysqli->prepare($insertSQL);
                                                    $stmt->bind_param("ss",$rolename,$autorisation);
                                                    $stmt->execute();
                                                    $stmt->close();
                                            
                                                $success_message = "Le rôle a été créé avec succès. Vous allez être redirigé vers la page d'accueil dans quelques secondes.";
