            ...
            <div id="layoutSidenav_content">
                ...
                <main>
                    <div class="container-fluid">
                        ...
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item active">Formulaire pour ajouter un résultat</li>
                        </ol>
                        <div class="row">
                            <form class="custom_display" id="form_crud" action="formulaire_resultat.php" method="post" enctype="multipart/form-data">
                                <div></div>
                                <div>
                                    <label for="resultName">Nom du résultat :</label>
                                    <input type="text" id="resultName" name="resultName" required>

                                    <label for="description">Description :</label>
                                    <textarea id="description" name="description" required></textarea>

                                    <label for="fileupload">Fichier résultat | Poid max. 2Mo :</label>
                ...
                <?php /************************ Si aucune connexion à la base de donnée, redirection vers la page d'erreur du serveur ******************************/
                                        ...
                                        if(isset($_POST['submit'])) {
                                            $resultname = trim($_POST['resultName']);
                                            $description = trim($_POST['description']);
                                            $fileupload = trim($_POST['fileupload']);
                                            $maxsize = 2097152; // 2mo = poid autorisé pour l'upload
                                            $allowed = array('gif', 'png', 'jpg', 'jpeg', 'pdf', 'docx'); // variables contenant un tableau des extensions autorisés
                                            ...
                                            if($resultname == '' || $description == '' ) {
                                                ...
                                                if ($isValid && is_numeric($resultname))  {
                                                    $isValid = false;
                                                    $error_message = "Le nom du fichier ne doit pas être numérique.";
                                                    echo '<p class="error">'.$error_message.'</p>';
                                                }
    
                                                // Vérification de la description du fichier
                                                if ($isValid && is_numeric($description))  {
                                                    $isValid = false;
                                                    $error_message = "La description du fichier doit contenir du texte.";
                                                    echo '<p class="error">'.$error_message.'</p>';
                                                }
    
                                                // Vérification de l'upload du fichier
                                                if ($isValid && $_FILES['fileupload']['error'] > 0) {
                                                $isValid = false;
                                                $error_message = "Erreur lors du transfert du fichier.";
                                                echo '<p class="error">'.$error_message.'</p>';
                                                }
                                                
                                                // Vérification du poid du fichier
                                                if ($isValid && $_FILES['fileupload']['size'] > $maxsize) {
                                                $isValid = false;
                                                $error_message = "Le fichier dépasse le poid maximum autorisé.";
                                                echo '<p class="error">'.$error_message.'</p>';
                                                }
                                                ...
                                                if (!in_array($ext, $allowed)) {
                                                    $isValid = false;
                                                    $error_message = "Le type de fichier n'est pas supporté.";
                                                    echo '<p class="error">'.$error_message.'</p>';
                                                }
                                           
                                                // Enregistrement dans la base de données
                                                if($isValid){
                                                    $insertSQL = "INSERT INTO resultat (libelle_resultat,description_resultat,fichier_resultat) values(?,?,?)";
                                                    $stmt = $mysqli->prepare($insertSQL);
                                                    $stmt->bind_param("sss",$resultname,$description,$fileupload);
                                                    $stmt->execute();
                                                    $stmt->close();
                                        
                                                    $success_message = "Le résultat a été créé avec succès. Vous allez être redirigé vers la page d'accueil dans quelques secondes.";
