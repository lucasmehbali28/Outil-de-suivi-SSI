            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid">
                        ...
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item active">Formulaire pour ajouter une règle</li>
                        </ol>
                        <div class="row">
                            <form class="custom_display" id="form_crud" action="formulaire_regles.php" method="post">
                                <div>
                                    </div><div>
                                        <label for="theme">Thèmes :</label>
                                        <select name="theme" id="theme" required>
                                            <option value="">--Selectionner un thème--</option>
                                            <?php $result = $mysqli->query('SELECT id_categorie, nom_categorie FROM categorie');
                                                while ($row = $result->fetch_array()): ?>
                                        ...
                                        <label for="controlDate">Date de contrôle :</label>
                                        <input type="date" id="controlDate" name="controlDate" required>

                                        <label for="ruleTitle">Nom :</label>
                                        <input type="text" id="ruleTitle" name="ruleTitle" required><br>

                                        <label for="reference">Référence :</label>
                                        <input type="reference" id="reference" name="reference" required>
                                        

                                        <label for="ruleGoal">Objectifs :</label>
                                        <input type="text" id="ruleGoal" name="ruleGoal" required>

                                        <label for="ruleDescription">Détails :</label>
                                        <input type="text" id="ruleDescription" name="ruleDescription" required> 

                                        <label for="action">Actions reliées :</label>
                ...
                <?php /************************ Si aucune connexion à la base de donnée, redirection vers la page d'erreur du serveur ******************************/
                                        ...
                                        if(isset($_POST['submit'])){
                                            ...
                                            $controldate = trim($_POST['controlDate']);
                                            $rulegoal = trim($_POST['ruleGoal']);
                                            $ruledescription = trim($_POST['ruleDescription']);
                                            ...
                                            if($ruletitle == '' || $controldate == '' || $rulegoal == '' || $ruledescription == '' || $theme == '' || $statute == '' || $action == '' || $reference == '' ){
                                            $isValid = false;
                                            $error_message = "Veuillez remplir tous les champs.";
                                            echo '<p class="error">'.$error_message.'</p>';
                                            }
                                        
                                            // Vérification des champs non numériques
                                            if($isValid && is_numeric($ruletitle) || is_numeric($rulegoal) || is_numeric($ruledescription) || is_numeric($action)) {
                                            $isValid = false;
                                            $error_message = "Vous devez saisir du texte.";
                                            echo '<p class="error">'.$error_message.'</p>';
                                            }
                                        
                                            // Vérification si la date est conforme
                                            if ($isValid && date('Y-m-d') > $controldate) {
                                                $isValid = false;
                                                $error_message = "La date de contrôle est antérieure à aujourd'hui.";
                                                echo '<p class="error">'.$error_message.'</p>';
                                            }
                                            
                                                // Enregistrement dans la base de données
                                                if($isValid){
                                                    $insertSQL = "INSERT INTO regle (categorie_regle,intitule_regle,objectif_regle,detail_regle,statut_regle,date_controle_regle,reference_regle,action_regle) values(?,?,?,?,?,?,?,?)";
                                                    $stmt = $mysqli->prepare($insertSQL);
                                                    $stmt->bind_param("ssssssss",$theme,$ruletitle,$rulegoal,$ruledescription,$statute,$controldate,$reference,$action);
                                                    $stmt->execute();
                                                    $stmt->close();
                                            
                                                $success_message = "La règle a été créé avec succès. Vous allez être redirigé vers la page d'accueil dans quelques secondes.";
