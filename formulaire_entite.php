                ...
                <main>
                    <div class="container-fluid">
                        ...
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item active">Formulaire pour intégrer une nouvelle entité</li>
                        </ol>
                        <div class="row">
                            <form class="custom_display" id="form_crud" action="formulaire_entite.php" method="post">
                                <div></div>
                                <div>
                                    <label for="entiteName">Nom de l'entité :</label>
                                    <input type="text" id="entiteName" name="entiteName" required>

                                    <label for="tel">Numéro de téléphone :</label>
                                    <input type="tel" name="tel" id="tel" pattern="^[0-9]{10}$" placeholder="Exemple: 0562626262" required>
                                    
                                    <label for="entiteAdress">Adresse :</label>
                                    <input type="texte" id="entiteAdress" name="entiteAdress" required>
                                    ...
                                    <?php /************************ Si aucune connexion à la base de donnée, redirection vers la page d'erreur du serveur ******************************/
                                        ...
                                        if(isset($_POST['submit'])){
                                            $entitename = trim($_POST['entiteName']);
                                            $tel = trim($_POST['tel']);
                                            $entiteadress = trim($_POST['entiteAdress']);
                                           
                                            $isValid = true;
                                        
                                            // Vérification si les champs sont remplis
                                            if($entitename == '' || $tel == '' || $entiteadress == '' ){
                                            $isValid = false;
                                            ...
                                            if($isValid && !is_numeric($tel)){
                                            $isValid = false;
                                            $error_message = "Le numéro de téléphone est invalide.";
                                            echo '<p class="error">'.$error_message.'</p>';
                                            }
                                        
                                            // Vérification si l'entite est valide
                                            if ($isValid && is_numeric($entitename)) {
                                                $isValid = false;
                                                $error_message = "L'entité est invalide.";
                                                echo '<p class="error">'.$error_message.'</p>';
                                            }
                                            
                                                if($isValid){
                                            
                                                    // Vérification si l'entite existe déjà dans la BDD
                                                    $stmt = $mysqli->prepare("SELECT nom_entite FROM entite WHERE nom_entite = ?");
                                                    $stmt->bind_param("s", $entitename);
                                                    $stmt->execute();
                                                    $result = $stmt->get_result();
                                                    $stmt->close();
                                                    // ATTENTION ! Il se peut que l'entite soit écrit en MAJ ou MIN et qu'elle ne retrouve pas le même nom !
                                                    if($result->num_rows > 0){
                                                        $isValid = false;
                                                        $error_message = "Cette entité existe déjà.";
                                                        echo '<p class="error">'.$error_message.'</p>';
                                                    }
                                                }
                                                ...
                                                if($isValid){
                                                    $insertSQL = "INSERT INTO entite(nom_entite,adresse_entite,telephone_entite) values(?,?,?)";
                                                    $stmt = $mysqli->prepare($insertSQL);
                                                    $stmt->bind_param("sss",$entitename,$entiteadress,$tel);
                                                    $stmt->execute();
                                                    $stmt->close();
                                            
                                                $success_message = "L'entité a été créé avec succès. Vous allez être redirigé vers la page d'accueil dans quelques secondes.";
